...
export default function EngineeringDirectory() {
  ...
  return <section id="engineering-directory" className="bg-white"><div className="content-wrap">
    ...
    <div className="discovery-grid">{visible.map(record => <article className="discovery-card" key={`${record.name}-${record.location}`}><div className="college-identity"><CollegeCrest name={record.name} /><div><h3>{record.name}</h3><p>{record.location}</p></div></div><span className="reference-badge">{record.scope === 'India' ? 'National directory' : record.scope}</span><dl className="reference-facts"><div><dt>Affiliation / status in source</dt><dd>{record.status}</dd></div></dl><details className="reference-source"><summary>Source & details to confirm</summary><p>Check current university affiliation, branch intake, entry route, approval where applicable, tuition and placement evidence.</p>{record.sources.map(source => <a key={source.file + source.page} href={source.url}>{source.file.replaceAll('_', ' ')} · Page {source.page}</a>)}</details><a className="text-link" href={`${contactHref}?subject=${encodeURIComponent(record.name + ' engineering admission enquiry')}`}>Discuss this institution <ArrowRight size={15} aria-hidden="true" /></a></article>)}</div>
