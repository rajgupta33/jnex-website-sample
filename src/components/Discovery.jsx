...
export function MedicalDiscovery() {
 return <section id="other-medical" className="bg-white"><div className="content-wrap"><p className="eyebrow">BEYOND MBBS</p><h2>Explore other medical admission pathways.</h2><p className="section-copy">JNEX also supports dental, AYUSH and BNYS, nursing, pharmacy, physiotherapy, allied health, paramedical, veterinary and research pathways.</p><div className="discovery-grid">{medical.map(([title, copy]) => <a className="discovery-card" key={title} href={'/medical-admissions/#' + title.toLowerCase().replaceAll(' ', '-')}><h3>{title}</h3><p>{copy}</p><span className="text-link">Explore {title} →</span></a>)}</div><a className="primary-button mt-8" href="/medical-admissions/">Explore Medical Admissions →</a></div></section>;
}
...
export function Guides() {
 ...
 if (!published.length) return <ResourceLibrary preview />;
