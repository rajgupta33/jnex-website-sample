...
export default function CollegeExplorer({ full = false, initialState = '', initialManagement = '' }) {
  ...
  return <section id="colleges" className="bg-slate-50"><div className="content-wrap">
    ...
    {records.length ? <div className="discovery-grid">{records.map(record => <article className="discovery-card medical-reference-card" key={record.id}>
      <div className="college-identity"><span className="college-monogram" aria-hidden="true">{record.name.split(/\s+/).filter(word => !["of", "and", "the"].includes(word.toLowerCase())).slice(0, 3).map(word => word[0]).join("")}</span><div><h3>{record.name}</h3><p>{record.city ? `${record.city}, ` : ""}{record.state}</p></div></div><div className="college-chips"><span>{record.management}</span><span>MBBS</span></div>
      <span className={`reference-badge ${record.verified ? 'verified' : ''}`}>{record.verified ? 'Verified record' : 'JNEX planning reference'}</span>
