...
export default function PanIndia({ full = false, editorial = false }) {
  ...
  function card(name) {
    ...
    return <a key={name} className="state-card" href={published ? `/mbbs-admission/${state.slug}/` : profileLink(name)} aria-label={published ? `MBBS Admission in ${name}` : `Explore MBBS pathways for a student from ${name}`} onClick={() => track('state_card_click', { state: name })}>
      <span><strong>{name}</strong>{editorial && <small>Review counselling routes, eligibility and college options with your profile.</small>}</span><ArrowUpRight size={17} aria-hidden="true" />
    </a>;
  }
  ...
  return <section id={editorial ? 'state-explorer' : 'states'} className={editorial ? 'bg-slate-50' : 'pan-india-section'}><div className="content-wrap">
    ...
    {editorial ? <p className="section-copy">Check counselling routes, college types, fees, previous cutoff trends, eligibility and official updates for the state you are considering.</p> : <><p className="section-copy">JNEX Education provides counselling and admission guidance for MBBS pathways across India. We help students understand Government, Private, Deemed University and other applicable admission routes based on NEET performance, domicile, category, budget and current counselling rules.</p><p className="section-copy">Explore state-wise admission information, college options, fees, cutoff trends, counselling authorities and eligibility before you build your preference list.</p><p className="coverage-line">All 28 States <span>•</span> State + AIQ Counselling <span>•</span> Union Territory pathways where applicable</p></>}
