...
export default function MedicalPathways() {
  return <><section id="career-pathways" className="bg-white"><div className="content-wrap">
    ...
    <div className="discovery-grid">{careerGroups.map(([title, programs, copy, href], index) => <article id={['bds', 'ayush', 'veterinary', 'nursing', 'rehabilitation', 'pharmacy', 'paramedical', 'research'][index]} className="discovery-card icon-card" key={title}>
      <div className="card-head"><TopicIcon title={title} /><span className="reference-badge">PATH {String(index + 1).padStart(2, '0')}</span></div><h3>{title}</h3><p className="course-programs">{programs}</p><p>{copy}</p>
    ...
    <div className="allied-course-grid">{alliedCourses.map((course, index) => <article id={course.name === 'Physiotherapy' ? 'physiotherapy' : `${course.name.toLowerCase().replaceAll(' ', '-')}-course`} className="allied-course-card" key={course.name}>
      <span className="course-index"><TopicIcon title={course.name} /><small>{String(index + 1).padStart(2, '0')}</small></span><div><h3>{course.name}</h3><p className="course-qualification">{course.qualification}</p><p className="course-duration">{course.duration}</p><p className="course-entry">{course.entry}</p>
